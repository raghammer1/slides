...
const presentationsChangeLogger = config => (set, get, api) => config(async function (nextState) {
  ...
  if (currentState.presentations !== newState.presentations) {
    ...
    const storeData = await (0, _api.getStore)(); // Assuming getStore() fetches the current store structure
    ...
    await (0, _api.setStore)(updatedData); // Sen
  }
}, get, api);
const usePresentationListStore = (0, _zustand.create)(presentationsChangeLogger((set, get) => ({
  presentations: [],
  addPresentation: newPresentation => {
    set(state => {
      ...
      return {
        presentations: [...state.presentations, presentationWithStandardizedSlide]
      };
    });
  },
  clearPresentations: () => set({
    presentations: []
  }),
  deleteOnePresentation: id => {
    console.log("Deleting presentation with ID: ".concat(id));
    set(state => {
      const updatedPresentations = state.presentations.filter(presentation => presentation.id !== id);
      return {
        presentations: updatedPresentations
      };
    });
  },
  updatePresentationTitle: (id, newTitle) => set(state => ({
    presentations: state.presentations.map(presentation => presentation.id === id ? {
      ...presentation,
      name: newTitle
    } : presentation)
  })),
  addSlide: (presentationId, newSlide) => {
    set(state => {
      ...
      const presentationIndex = state.presentations.findIndex(p => p.id === presentationId);
      ...
      const newSlideNumber = updatedPresentation.slides.reduce((max, slide) => Math.max(max, slide.slideNumber || 0), 0) + 1;
  ...
  deleteSlide: (presentationId, slideId) => {
    set(state => {
      ...
      const updatedSlides = updatedPresentation.slides.filter(slide => slide.id !== slideId).map((slide, index) => ({
        ...slide,
        slideNumber: index + 1
      }));
  ...
  addElementToSlide: (presentationId, slideId, newElement) => {
    set(state => {
      ...
      const slideIndex = updatedPresentation.slides.findIndex(slide => slide.id === slideId);
      if (slideIndex === -1) {
      ...
      updatedSlide.elements = updatedSlide.elements.map(element => {
        if (element.id === elementId) {
          return {
            ...
            top,
            left
          }; // Update the targeted element's position
        }
  ...
  deleteElementFromSlide: (presentationId, slideId, elementId) => {
    set(state => {
      ...
      updatedSlide.elements = updatedSlide.elements.filter(element => element.id !== elementId);
...
const initializeStore = async () => {
  const initialStore = await (0, _api.getStore)();
  usePresentationListStore.setState({
    presentations: initialStore.store.presentations ? initialStore.store.presentations : []
  });
  console.log("".concat(initialStore.store.presentations ? initialStore.store.presentations : {}));
};
exports.initializeStore = initializeStore;
var _default = exports.default = usePresentationListStore;